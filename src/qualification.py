...
class QualificationFlow:
    ...
    def on_start_interview(self, max_rounds, self_rating_input=0):
        ...
        if not self.target_text:
            return [
                {
                    "role": "assistant",
                    "content": "❌ Error: No Target Text found. Please complete Step 1 first.",
                }
        ...
        if self_rating_input and float(self_rating_input) > 0:
            ...
            return [
                {
                    "role": "assistant",
                    "content": f"✅ Assessment Skipped (Self-Rating).\n**Score:** {score}\n**Reason:** {self.reason}",
                }
            ], gr.update(interactive=False)

        print(
            f"DEBUG: Starting Interview. Target Text ({len(self.target_text)} chars): {self.target_text[:50]}..."
        )
        ...
        try:
            ...
            if status == "asking":
                ...
                return [
                    {"role": "assistant", "content": f"🤖 Examiner: {question}"}
            ...
            return [
                {"role": "assistant", "content": f"Error calling LLM: {str(e)}"}
            ], gr.update(interactive=False)
    ...
    def on_user_reply(self, user_input, history):
        ...
        try:
            ...
            difficulty = data.get(
                "difficulty"
            )  # Integer 1, 3, 5, 10 for the NEXT question
            self_rating = data.get("self_rating")  # Phase 3 extraction

            # DEBUG: Print parsed values
            print(
                f"DEBUG: phase={new_phase}, answer_quality={answer_quality}, prev_diff={self.previous_difficulty}, current_score={self.score_accumulator}"
            )
                ...
                try:
                    ...
                    if self.current_phase == "1" or self.score_accumulator == 0:
                        ...
                        self.internal_history.append(
                            f"[System]: Base Score set to {b_score}"
                        )
            ...
            if status == "finished":
                if self.current_phase == "3" or self.current_phase == "3_done":
                    ...
                    should_stop = (
                        True  # Allow finish if LLM decides so (likely user quit)
                    )
                    stop_reason = reason
            ...
            if not should_stop:
                ...
                history.append(
                    {
                        "role": "assistant",
                        "content": f"🤖 Examiner: {question}{diff_str}{score_str}",
                    }
                )
                return history, ""
                ...
                if self_rating is not None:
                    try:
                        ...
                        self.internal_history.append(
                            f"[System]: Averaging Probe ({self.score_accumulator}) + Self-Rating ({s_rating}) = {final_score}"
                        )
                    except:
                        pass
                ...
                score_progression = " -> ".join(
                    [f"{s:.1f}" for s in self.score_history]
                )
